...
thil_2015_event<- read.xlsx("evenement.xlsx", sheet = 1, startRow = 1, colNames = TRUE, 
                            rowNames = FALSE, detectDates = F, skipEmptyRows = FALSE,
                            rows = NULL, cols = NULL, check.names = FALSE, namedRegion = NULL)

thil_2015_event[,2] <-as.POSIXct( round(convertToDateTime(thil_2015_event[,2]),"mins"),format="%d/%m/%Y %H:%M",tz="GMT")
thil_2015_event[,3] <-as.POSIXct( round(convertToDateTime(thil_2015_event[,3]),"mins"),format="%d/%m/%Y %H:%M",tz="GMT")


reserve_2015_event<- read.xlsx("evenement.xlsx", sheet = 2, startRow = 1, colNames = TRUE, 
                               rowNames = FALSE, detectDates = F, skipEmptyRows = FALSE,
                               rows = NULL, cols = NULL, check.names = FALSE, namedRegion = NULL)

reserve_2015_event[,2] <-as.POSIXct( round(convertToDateTime(reserve_2015_event[,2]),"mins"),format="%d/%m/%Y %H:%M",tz="GMT")
reserve_2015_event[,3] <-as.POSIXct( round(convertToDateTime(reserve_2015_event[,3]),"mins"),format="%d/%m/%Y %H:%M",tz="GMT")


## création de la charte visuelle pour matérialiser les évenement (création d'instance layer)

n <- length(thil_2015_event$Id)
for (i in 1:n){
  eval(parse(text=paste0("a_thil",i," <- thil_2015_event[",i,",2]")))
  eval(parse(text=paste0("b_thil",i," <- thil_2015_event[",i,",3]")))
...
###
n <- length(reserve_2015_event$Id)
for (i in 1:n){
  eval(parse(text=paste0("a_reserve",i," <- reserve_2015_event[",i,",2]")))
  eval(parse(text=paste0("b_reserve",i," <- reserve_2015_event[",i,",3]")))
...
###  cadrage temporel (on veut un graph par trimestre) ### a améliorer
  ...
  for (i in 1:n){
    ...
    eval(parse(text=paste0("p_T",i," <- ggplot() + 
                           ...
                           geom_point(data = ",site,"_",annee,"_event_",param,",
                                      aes(x = date, y =Signale_amplitude,colour=Intensite_signal_amplitude),
                                      size=3)+
...
graph_T_cond <- function(site,annee,param){
  ...
  for (i in 1:n){
    event <- eval(parse(text=paste0("viz_event_",site,"")))
    eval(parse(text=paste0("p_T",i," <- ggplot() + 
                           geom_line (data = ",site,"_",annee,"_event_",param,", aes(x = date, y = non_signalees)) +
...
## modification de la fonction de création de grapj pour qu'lle inclue également la matérialisation des évenements

#####
graph_T_max_E <- function(site,annee,param){
  x11()
  for (j in 1:4){        ## nombr de trimestre dans chaque année - utilisation de j car i est déjà apellé dans "viz event"
    event <- eval(parse(text=paste0("viz_event_",site,"")))
    eval(parse(text=paste0("p_T",j," <- ggplot() + 
                           geom_line (data = ",site,"_",annee,"_event_",param,", 
                           aes(x = date, y = filtre)) +
                           geom_point(data = ",site,"_",annee,"_event_",param,",
                           aes(x = date, y =Signale_niveau,colour=Intensite_signal_niveau),size=3)+
                           scale_color_gradient(low=\"red\", high=\"green\") +
                           ",event," +
                           xlim(daterange_",annee,"$T",j,") ")
                           ))
  }
  liste_temp <- paste0("p_T",paste0(1:4))
  liste_graph <-paste(liste_temp,collapse=",")
  eval(parse(text=paste0("plot_grid(",liste_graph,", nrow=",4,", align = \"v\")")))
  }
######

graph_T_max_E("thil", "2015", "ox")
graph_T_max_E("reserve", "2015", "ox")
